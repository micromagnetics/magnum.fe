...
class DemagField:
  ...
  def _assemble(self):
    ...
    a = inner(grad(v), grad(u))      * dx(1)    \
      + inner(grad(v), grad(u))      * dx(1000) \
      + inner(grad(v), gx * grad(u)) * dx(1001) \
      + inner(grad(v), gy * grad(u)) * dx(1002) \
      + inner(grad(v), gz * grad(u)) * dx(1003)

    L = inner(grad(v), m) * dx(1)
