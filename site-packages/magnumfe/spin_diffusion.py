...
class SpinDiffusion(object):
  ...
  def step(self, state, dt):
    ...
    pde = inner(s_diff, zeta) * state.dx(Omega) +                                                                       \
        + D0x2 * inner(grad(s), nabla_grad(zeta)) * state.dx(Omega)                                                     \ 
        - D0x2 * beta * beta_prime * inner(outer(state.m, nabla_grad(s) * state.m), nabla_grad(zeta)) * state.dx(omega) \
        + D0x2 * L**2 / lambda_sf**2 * inner(s , zeta) * state.dx(Omega)                                                \
        + D0x2 * L**2 / lambda_j**2  * inner(cross(s, state.m), zeta) * state.dx(omega)                                 \
        - beta * Constant(Constants.mu_b / Constants.e) * inner(outer(state.m, j), nabla_grad(zeta)) * state.dx(omega)
