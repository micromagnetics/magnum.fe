...
class SpinDiffusion(object):
  ...
  def step(self, state, dt):
    ...
    a = inner(s, zeta) / Constant(dt) * state.dx(Omega)                                                                 \
        + D0x2 / Constant(self.scale**2) * inner(nabla_grad(s), nabla_grad(zeta)) * state.dx(Omega)                                               \
        - D0x2 / Constant(self.scale**2) * beta * beta_prime * inner(outer(state.m, nabla_grad(s) * state.m), nabla_grad(zeta)) * state.dx(omega) \
    ...
    L = inner(state.s, zeta) / Constant(dt) * state.dx(Omega)                                                           \
        + beta * Constant(Constants.mu_b / Constants.e / self.scale) * inner(outer(state.m, state.j), nabla_grad(zeta)) * state.dx(omega)
