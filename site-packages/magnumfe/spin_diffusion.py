...
class SpinDiffusion(object):
  ...
  def step(self, state, dt):
    ...
    a = inner(s, zeta) / Constant(dt) * state.dx(Omega)                                                                 \
        + D0x2 * inner(nabla_grad(s), nabla_grad(zeta)) * state.dx(Omega)                                               \
    ...
    L = inner(state.s, zeta) / Constant(dt) * state.dx(Omega)                                                           \
        + beta * Constant(Constants.mu_b / Constants.e) * inner(outer(state.m, state.j), nabla_grad(zeta)) * state.dx(omega)
