...
class FredkinKoehler(LLGTerm):
  ...
  def __init__(self, rhs_form, rhs_func):
    """
    *Arguments*
  ...
  def calculate_u1(self, rhs_func, V):
    ...
    if not hasattr(self, '_assemble_cache_u1'):
      ...
      L = self.rhs_form(cache.rhs_func, v, n)
    ...
    cache.rhs_func.assign(rhs_func)
  ...
  def calculate(self, state):
    ...
    *Arguments*
    ...
    rhs_func = cache.mesh.cut(self.rhs_func(state)) if state.mesh_has_domains() else self.rhs_func(state)

    u1 = self.calculate_u1(rhs_func, cache.V)
