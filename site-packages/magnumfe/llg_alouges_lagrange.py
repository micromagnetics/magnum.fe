...
class LLGAlougesLagrange(object):
  r"""
  Solver for the Landau-Lifshitz-Gilbert equation according to Alouges [1]. In this two-step algorithm :math:`\partial_t \vec{m} = \vec{v}` is computed as
  ...
  where both the trial and test functions are restricted to :math:`T_{\vec{m}}` which is the tangent space of :math:`\vec{m}` [2]. As suggested in [3] the exchange field is integrated implicitly whereas all other contributions to the effective field :math:`\vec{H}_\text{eff}` are integrated explicitly. This class implements the tangent-space restriction through a Lagrange multiplier ansatz as proposed in [3]
  ...
  .. note:: This class requires CBC.Block and the EPetra backend to be enabled.
