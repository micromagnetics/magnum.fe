...
class LLG4:
  ...
  def __init__(self, mesh, material, **kwargs):
    ...
    *Arguments*
    ...
    transformation_order = 2 #if (self.order == 1) else 2
  ...
  def calculate_dm(self, m, dt, **kwargs):
    ...
    *Arguments*
    ...
    w1    = TestFunction(self.VV)
    w2    = TestFunction(self.VD)
    ...
    # Bilinear form
    a11  = self.material.alpha * dot(v, w1) * dx + dot(cross(m, v), w1) * dx        # LLG
    a11 += - 0.5 * dt * f_ex * Dx(v[i],j) * Dx(w1[i],j) * dx                        # Exchange
    a12  = - self.material.ms * self.material.gamma * inner(grad(u), w1) * dx       # Demag
    a13  = sigma * inner(m, w1) * dx                                                # Lagrange Multipliers
    a21  = - 0.5 * dt * self.material.ms * inner(v, grad(w2)) * dx(0)               # Demag Field, Implicit RHS
    a22  = Dx(w2, i) * Dx(u, i)                * dx(0) + \
           Dx(w2, i) * Dx(u, i)                * dx(1) + \
           Dx(w2, i) * self.gx[i,j] * Dx(u, j) * dx(2) + \
           Dx(w2, i) * self.gy[i,j] * Dx(u, j) * dx(3) + \
           Dx(w2, i) * self.gz[i,j] * Dx(u, j) * dx(4)                              # Demag Field, Stiffness
    a31  = inner(m, v) * w3 * dx                                                    # Constraint

    # Linear form
    L1 = f_ex * Dx(m[i],j) * Dx(w1[i],j) * dx          # Exchange
    L2 = self.material.ms * inner(m, grad(w2)) * dx(0) # Demag Field
    ...
    #######################################################
    bc22 = DirichletBC(self.VD, Constant(0.0), DomainBoundary())
    ...
    A13 = assemble(a13)
    A21 = assemble(a21)
    A22, A22n = symmetric_assemble(a22, bc22)
    A31 = assemble(a31)

    A = block_mat([[A11, A12, A13],
                   [A21, A22,   0],
                   [A31,    0,  0]])

    b1  = assemble(L1)
    b2  = assemble(L2)
    b3  = A13.create_vec() # is this zero?

    b = block_vec([b1, b2, b3])

    #######################################################
    # Solve the system
    #######################################################

    # TODO setup some preconditioner
