...
class DemagField(LLGTerm):
  ...
  def _assemble(self, state):
    ...
    VS = FunctionSpace(mesh, 'CG', self.order) # TODO from state?
    ...
    # TODO use all instead of magnetic in a

    a = inner(grad(v), grad(u))      * state.dx('magnetic', measure) \
    ...
    L = inner(grad(v), state.m) * state.dx('magnetic', measure)

    bc = DirichletBC(VS, Constant(0.0), DomainBoundary())

    u = Function(VS)
    #print state.mesh.domains().markers(3)
    #print mesh.domains().markers(3)
  ...
  def calculate_potential(self, state):
    ...
    *Arguments*
    ...
    if isinstance(state.mesh, WrappedMesh):
      Im.assign(state.mesh.expand(state.m))
    else:
      Im.assign(state.m)
  ...
  def create_mesh(*args, **kwargs):
    ...
    *Arguments*
        ...
        domains
            Domain IDs the mesh is cut/expanded to. Defaults to 1.
    """
    ...
    domains = kwargs.pop('domains', 1)
    mesher.create_shell(d, **kwargs)

    mesh    = mesher.mesh(scale)

    return WrappedMesh.create(mesh, domains), mesher.get_sample_size(scale = scale)
