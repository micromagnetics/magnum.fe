...
class DemagField(LLGTerm):
  ...
  def __init__(self, sample_size, order = 1, mesh = None):
    """
    ...
    *Arguments*
        ...
        mesh
            The mesh. If not provided it is taken from magnetization input.
    """
  ...
  def _assemble(self, state):
    ...
    VS = FunctionSpace(state.mesh, 'CG', self.order)
    VV = VectorFunctionSpace(state.mesh, 'CG', 1)
    ...
    # TODO use all instaed of magnetic in a
    a = inner(grad(v), grad(u))      * state.dx('magnetic') \
      + inner(grad(v), grad(u))      * state.dx(1000) \
      + inner(grad(v), gx * grad(u)) * state.dx(1001) \
      + inner(grad(v), gy * grad(u)) * state.dx(1002) \
      + inner(grad(v), gz * grad(u)) * state.dx(1003)

    L = inner(grad(v), state.m) * state.dx('magnetic')
  ...
  def form_term(self, state, v, dx):
    return grad(self.calculate_potential(state))
  ...
  def calculate_potential(self, state):
    ...
    *Arguments*
    ...
    # TODO interpolate if shell mesh was provided in constructor
    solver, assembler, A, b, bc, Im, u = self._assemble(state)
  ...
  def create_mesh(*args, **kwargs):
    ...
    *Arguments*
    ...
    #return WrappedMesh.create(mesh, 1, {'sample_size': mesher.get_sample_size(scale=scale)})

    return mesh, mesher.get_sample_size(scale=scale)
