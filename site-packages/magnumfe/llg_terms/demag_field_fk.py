...
class DemagFieldFK(LLGTerm):
  ...
  def calculate_u2_bc(self, state, u1):
    ...
    if not hasattr(self, '_assemble_cache_u2_bc'):
      ...
      cache.pwiseConstants = createPiecewiseConstantScalarSpace(cache.context, grid)
      cache.pwiseLinears = createPiecewiseLinearContinuousScalarSpace(cache.context, grid)

      # Construct elementary operators
      cache.dlpOp = createLaplace3dDoubleLayerBoundaryOperator(cache.context, cache.pwiseLinears, cache.pwiseLinears, cache.pwiseConstants)
      cache.idOp = createIdentityOperator(cache.context, cache.pwiseLinears, cache.pwiseLinears, cache.pwiseConstants)
