...
class LLG:
  ...
  def calculate_dm(self, m, dt, u_demag):
    ...
    f_ex    = (- 2.0 * self.scale * self.material.Aex * self.material.gamma) / \
    ...
    # assemble constraint
    B  = DofAssembler.assemble(ScalarProductMatrix2(VS, VV, m))
    BT = DofAssembler.assemble(TransScalarProductMatrix(VS, VV, m))

    #B  = assemble_scalar_product(m.vector(), VV, VS)
    #BT = assemble_transp_scalar_product(m.vector(), VV, VS)
