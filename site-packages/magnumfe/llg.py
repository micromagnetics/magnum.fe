...
class LLG:
  ...
  def calculate_v(self, state, dt):
    ...
    *Arguments*
    ...
    VV = VectorFunctionSpace(state.mesh, 'CG', 1, 3) # TODO cache
    VS = FunctionSpace(state.mesh, 'CG', 1)          # TODO cache
    ...
    # Forms
    #######################################################
    f_ex    = (- 2.0 / self.scale**2 * state.material.Aex * state.material.gamma) / \
              (Constants.mu0 * state.material.ms)

    # Bilinear form
    a  = Constant(state.material.alpha) * dot(v, w) * dx + dot(cross(state.m, v), w) * state.dx('magnetic') # LLG
    a += Constant(-0.5 * dt * f_ex) * Dx(v[i],j) * Dx(w[i],j) * state.dx('magnetic')                        # Exchange

    # Linear form
    L  = Constant(f_ex) * Dx(state.m[i],j) * Dx(w[i],j) * state.dx('magnetic')                              # Exchange
    for term in self.terms:                                                                                 # Additional Term
  ...
  def step(self, state, dt):
    ...
    *Arguments*
    ...
    return Function(VV, DofAssembler.assemble(NormalizedVector(VV, m)))
