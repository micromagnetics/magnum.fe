...
from . import Material, WrappedMesh
...
def _normalize(self, graceful = True):
  ...
  assemble(inner(v, expr) * dP, self.vector())
  ...
  def M_inv_diag(self, domain = "all"):
    ...
    *Returns*
      :class:`dolfin.Matrix`
        the matrix
    """
    if not hasattr(self, '_M_inv_diag'):
      ...
      self._M_inv_diag = assemble(inner(v, u) * dP)
      self._M_inv_diag.zero()
      self._M_inv_diag.set_diagonal(diag)

    return self._M_inv_diag
